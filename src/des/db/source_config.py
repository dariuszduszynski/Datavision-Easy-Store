...
class ColumnMapping(BaseModel):
    ...
    metadata_columns: Dict[str, str] = Field(
        default_factory=dict, description="Additional columns to store in metadata"
    )
...
class DatabaseConnection(BaseModel):
    ...
    db_schema: Optional[str] = Field(
        None, description="Schema name (Oracle, PostgreSQL)", alias="schema"
    )
    ...
    driver_options: Dict[str, Any] = Field(
        default_factory=dict, description="Driver-specific connection options"
    )
...
class SourceTableConfig(BaseModel):
    ...
    table_schema: Optional[str] = Field(
        None, description="Schema name (if applicable)", alias="schema"
    )
    ...
    where_clause: Optional[str] = Field(
        None, description="Optional WHERE clause for filtering (without WHERE keyword)"
    )

    # Claim logic
    status_pending_value: str = Field(
        "pending", description="Value in status column indicating pending files"
    )
    status_claimed_value: str = Field(
        "claimed", description="Value to set when claiming files"
    )

    # Shard routing
    shard_key_column: Optional[str] = Field(
        None, description="Column to use for shard routing (if not using s3_key)"
    )
...
class SourceDatabaseConfig(BaseModel):
    ...
    claim_timeout_seconds: int = Field(
        300, description="Timeout for claimed files (before they can be reclaimed)"
    )

    # Shard routing
    shard_bits: int = Field(
        8, ge=1, le=16, description="Number of shard bits (2^n shards)"
    )
