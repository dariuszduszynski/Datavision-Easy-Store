...
- **`cache.py`** – Cache interface `IndexCacheBackend` with implementations:
  - `InMemoryIndexCache` (thread-safe, optional TTL, LRU eviction),
  - `RedisIndexCache` (JSON serialization with TTL support),
  - `NullCache` (no-op).
